...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Convection plotting')
    ...
    parser.add_argument('--beta', type=float, default=10.0, help='Shift parameter for gate')
    ...
    parser.add_argument('--colorFromGridOnly', dest='colorFromGridOnly', action='store_true')
    ...
    Convectionpath ='/home/elhamod/projects/deepxdeplayground/AE/Convection'
    ...
    ####Loss
    loss_obj = Loss(device, args.xgrid, args.nt, args.beta, args.u0_str, args.N_f)
    ...
    for i in range(trajectory_models.shape[0]):
        ...
        model_repopulated = repopulate_model(model_flattened, get_PINN(args.layers, args.activation, device))
        model_repopulated.eval()
        model_repopulated = model_repopulated.to(device)
        loss =  loss_obj.get_loss(model_repopulated, loss_type).detach()
    ...
    if args.vmax <= 0 or args.vmin <= 0:
        if args.colorFromGridOnly == False:
            if args.vmax <= 0:
    ...
    for plot_ in plots_:
        ...
        scatter = plt.scatter(trajectory_coordinates[:, 0].detach().cpu().numpy(), trajectory_coordinates[:, 1].detach().cpu().numpy(), c=c, marker='o', s=9, norm=norm, cmap=cmap, zorder=100) 
