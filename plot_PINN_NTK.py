...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Wang et. al. plotting')
    ...
    for i in range(trajectory_models.shape[0]):
        ...
        model_repopulated = repopulate_model(model_flattened, get_PINN(PINN_layers, device))
        model_repopulated.eval()
        model_repopulated = model_repopulated.to(device)
        loss =  get_errors(model_repopulated, loss_type).detach()
